describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    const sRoot = await prodItems[0].getProperty('shadowRoot');
    const btn = await sRoot.$("button");
    await btn.click();
    const txt = await btn.getProperty("innerText");
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const txt = await cart.getProperty("innerText");
    expect(txt['_remoteObject'].value).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cart = await page.$('#cart-count');
    const txt = await cart.getProperty("innerText");
    expect(txt['_remoteObject'].value).toBe("0");
  }, 10000);
